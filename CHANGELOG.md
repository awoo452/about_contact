# Changelog
All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/), and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [0.0.10] - 2026-03-30
### Changed
- Updated mcp to 0.10.0 to address CVE-2026-33946.

## [0.0.9] - 2026-03-27
### Changed
- Updated dependencies: rails 8.1.3, solid_queue 1.4.0, thruster 0.1.20, action_text-trix 2.1.18, json 2.19.3, bigdecimal 4.1.0, net-imap 0.6.3.

## [0.0.8] - 2026-03-21
### Changed
- Standardized README sections to the shared format.

## [0.0.7] - 2026-03-21
### Changed
- Standardized changelog dates to YYYY-MM-DD.

## [0.0.6] - 2026-03-21
### Changed
- Updated GitHub Actions dependencies: actions/cache v5, actions/upload-artifact v7.

## [0.0.5] - 2026-03-19
### Changed
- Refreshed Gemfile.lock via bundle update for Ruby 4.0.2.

## [0.0.4] - 2026-03-19
### Changed
- Pinned Ruby to 4.0.2 across runtime files and Gemfile.lock.

## [0.0.3] - 2026-03-19
### Removed
- Removed application CSS and stylesheet tag to keep the template backend-only.
- Removed view class attributes to keep the template backend-only.

## [0.0.2] - 2026-03-19
### Added
- About, contact, and FAQ pages wired to JSON content
- Contact form mailer and SMTP configuration hooks
- Minimal system test setup to keep CI green

## [0.0.1] - 2026-03-19
### Added
- Basic rails app
